...
class WebServer extends Worker
{
    ...
    public function onMessage($connection)
    {
        ...
        if ($workerman_file_extension === '') {
            $workerman_path           = ($len = strlen($workerman_path)) && $workerman_path[$len - 1] === '/' ? $workerman_path . 'user.php' : $workerman_path . '/user.php';
        ...
        if ($workerman_file_extension === 'php' && !is_file($workerman_file)) {
            $workerman_file = "$workerman_root_dir/user.php";
            if (!is_file($workerman_file)) {
                $workerman_file           = "$workerman_root_dir/user.html";
