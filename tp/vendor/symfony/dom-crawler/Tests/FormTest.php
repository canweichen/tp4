...
class FormTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $formHtml = '<form id="login_form" action="https://login.foo.com/login.html?login_attempt=1" method="POST"><input type="text" name="foo" value="foo" /><input type="submit" /></form>';

        $form = $this->createForm($formHtml);
        $this->assertEquals('https://login.foo.com/login.html?login_attempt=1', $form->getUri(), '->getUri() returns absolute URIs set in the action form');
